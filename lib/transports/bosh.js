...
function retryRequest(opts, timeout, allowedRetries) {
    return timeoutPromise(makeRequest(opts), (timeout || 20) * 1000).then(function (result) {
        ...
        if (allowedRetries > 0) {
            ...
            const err = new Error('Dead Connection, exceeded retry limit');
            err.status = 500;
            throw err;
        }
    });
...
BOSHConnection.prototype.send = function (data) {
    ...
    if (self.hasStream) {
        ...
        setTimeout(self.longPoll.bind(self), 0);
    }
};
...
BOSHConnection.prototype.request = function (bosh) {
    ...
    var req = retryRequest({
        ...
        credentials: self.config.credentialsMode,
        ...
        if (err && (err.status === 404 || err.status === 500)) {
            ...
            if (self.config.transport === 'websocket') {
                self.sm.allowResume = false;
            }
            self.disconnect();
            throw new Error('Application Chat Server Session Closed');
        }
        else {
            self.hasStream = false;
    ...
    req.then(function (body) {
        if (body === undefined || body === null) {
            throw new Error('Response body cannot be (object) empty');
        }
